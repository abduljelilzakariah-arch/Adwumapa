...
const JobCard = ({ job }) => {
  const orgName = job.company || 'Company'

  return (
    <article className="card listing-card h-100 position-relative">
      {job.isFeatured && <div className="featured-ribbon">FEATURED</div>}
      <div className="card-body p-4">
        <div className="d-flex gap-3 mb-3">
          ...
          <div className="flex-grow-1 min-w-0">
            <div className="d-flex flex-wrap gap-1 mb-1">
              ...
              {job.isNew && <span className="badge badge-new rounded-pill">New</span>}
              {job.isUrgent && <span className="badge badge-urgent rounded-pill">Urgent</span>}
            </div>
            <h5 className="card-title fw-bold mb-1 text-truncate">
              <Link to={`/listing/${job.id}`} className="text-decoration-none text-dark stretched-link">
                {job.title}
              </Link>
            </h5>
        ...
        <div className="d-flex justify-content-between align-items-center">
          <span className="fw-bold text-adwuma-green">{formatPay(job)}</span>
